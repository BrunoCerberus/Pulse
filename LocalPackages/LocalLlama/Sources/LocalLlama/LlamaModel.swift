...
class LlamaModel {
    ...
    init(path: String, configuration: Configuration = .init()) throws {
        ...
        // Get vocabulary pointer using new API
        guard let vocab = llama_model_get_vocab(model) else {
            llama_model_free(model)
            throw SwiftLlamaError.others("Cannot get vocabulary from model")
        }
        self.vocab = vocab

        // Verify vocabulary is accessible using new API
    ...
    private func tokenToCChars(token: llama_token) -> [CChar] {
        var length: Int32 = 8
        var piece = [CChar](repeating: 0, count: Int(length))

        // Use vocab pointer for token_to_piece
