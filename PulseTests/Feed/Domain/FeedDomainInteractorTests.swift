...
struct FeedDomainInteractorTests {
    ...
    @Test("Loading fresh articles does not start digest generation")
    func loadDataDoesNotAutoGenerateDigest() async {
        mockNewsService.topHeadlinesResult = .success(Article.mockArticles)
        ...
        #expect(sut.currentState.generationState == .idle)
    }
    ...
    @Test("Preload model triggered when a local model is available")
    ...
    @Test("Preload model is skipped when no local model is available")
    func preloadSkippedWithoutLocalModel() async {
        mockFeedService.cachedDigest = nil
