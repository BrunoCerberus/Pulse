...
- Article `mediaURL` → inline `VideoPlayerView` `WKWebView`. HTTPS-gated via
  `VideoPlayerView.isSafeInlineVideoURL` (non-`https` schemes never reach `load(_:)`),
  matching the open-in-browser gates (`MediaDetailDomainInteractor.openInBrowser`,
  `ArticleDetailDomainInteractor.externalURL`). Found by the weekly sweep
  (`SWEEP-2026-06-03-01`).
