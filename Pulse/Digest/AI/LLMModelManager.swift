...
// MARK: - LLM Model Manager
    ...
    /// Mac Catalyst stub. The vendored `swift-llama-cpp` xcframework ships no
    /// Catalyst slice, so on-device LLM inference is unavailable on Mac. All
    /// entry points return `LLMError.serviceUnavailable` to keep premium LLM
    /// features gracefully disabled on Catalyst.
    final class LLMModelManager: @unchecked Sendable {
        static let shared = LLMModelManager()
        ...
        func loadModel(progressHandler _: @escaping @Sendable (Double) -> Void) async throws {
        ...
        func generate(
            prompt _: String,
            systemPrompt _: String = LLMModelManager.defaultSystemPrompt,
            maxTokens _: Int,
