...
- **Sign-out and account deletion** call `SettingsViewModel.clearAllUserData()` — a single-flight (`isClearingUserData`) wipe of SwiftData + L1/L2 cache + Keychain + UserDefaults + ThemeManager + widget/Share-Extension App Group data + TTS Live Activity + personalization profile & engagement queue + app-lock state, plus an APNs unregister and a best-effort private CloudKit zone delete (so reinstall-restore can't resurrect erased PII). A **third trigger** fires the same wipe: `AuthenticationManager` (wired in `PulseSceneDelegate.configureSessionCleanup`) runs it on any authenticated→unauthenticated transition (server-driven sign-out / remote account disable), de-duped by the single-flight guard.
- **Reviewer-only anonymous sign-in** — `SignInView` has a hidden 5-tap-on-logo gesture that calls `Auth.auth().signInAnonymously()` so App Review reaches auth-gated screens without shared OAuth credentials (avoids 2FA friction). Trigger lives in App Store Connect → App Review Information. Hardened to keep reviewer sessions off the production analytics surface: a 60s throttle (UserDefaults key `pulse.anonymousSignInLastAttempt`, recorded only on confirmed Firebase success, survives sign-out) rejects rapid re-taps with `AuthError.anonymousSignInThrottled`, which `AuthDomainInteractor` special-cases to suppress the failed-`signIn` analytics + `recordError` calls; `PulseSceneDelegate.configureAnalyticsUserID()` sends `setUserID(nil)` for `provider == .anonymous` so no analytics identity is bound. The successful `signIn(provider: "anonymous")` event is still logged for filtering. Implementation in `LiveAuthService+Anonymous.swift`.
...
- **Security review is threat-model-guided** (separate from the privacy gates above). `THREAT_MODEL.md` + `SEVERITY_RUBRIC.md` at repo root scope the trust boundaries, untrusted inputs (third-party RSS article JSON → on-device Gemma is the primary sink), vuln classes, and severity scoring (Medium default ceiling — most flows sit behind Firebase auth + a private CloudKit zone). `.github/workflows/claude-security-review.yml` runs an **advisory** two-stage pass on PRs — `pr-discovery` (emits findings, posts nothing) → separate-context `pr-verify` (disproves each, posts only confirmed) — plus a weekly sweep, distinct from the general `claude-code-review.yml`. Keep these advisory (not in the required-check ruleset); when you patch a validated finding, update `THREAT_MODEL.md` and add a regression test. Full rules: AGENTS.md #35.
...
| **Security** | `LiveAppLockService.swift`, `APIKeysProvider.swift` (Keychain storage for API keys), `PromptSanitizer.swift` (`Configs/AI/`), `SafeMediaURL.swift` (`Configs/Networking/`), `PrivacyInfo.xcprivacy` | Biometric + passcode app lock; Keychain accessed inline via the `Security` framework; `PromptSanitizer` = untrusted-RSS-text → on-device-Gemma prompt-injection gate (all three prompt builders); `SafeMediaURL` = HTTPS-only gate shared across every media/article URL sink |
