...
final class PulseSearchUITests: BaseUITestCase {
    ...
    func testSearchFlow() {
        ...
        XCTAssertTrue(searchNav.safeWaitForExistence(timeout: Self.defaultTimeout), "Search navigation should load")
        ...
        if searchField.exists {
            ...
            XCTAssertTrue(keyboard.safeWaitForExistence(timeout: 3), "Keyboard should appear for text input")
        ...
        XCTAssertTrue(searchField.safeWaitForExistence(timeout: 5))
        ...
        let clearButton = app.searchFields.buttons["Clear text"]
        if clearButton.safeWaitForExistence(timeout: 3) {
            clearButton.tap()
        ...
        if cancelButton.safeWaitForExistence(timeout: 3) {
            cancelButton.tap()
            XCTAssertTrue(!app.keyboards.element.safeWaitForExistence(timeout: 1), "Keyboard should dismiss after cancel")
        }
        ...
        if articleCardsForNavigation.count > 0 {
            ...
            let didNavigate = backButton.safeWaitForExistence(timeout: 5)

            if didNavigate {
                backButton.tap()
                XCTAssertTrue(searchField.safeWaitForExistence(timeout: 5), "Should return to Search")
            }
        }
        ...
        searchField.tap()
        let keyboard = app.keyboards.element
        XCTAssertTrue(keyboard.safeWaitForExistence(timeout: 3), "Keyboard should appear")
        ...
        XCTAssertTrue(searchField.safeWaitForExistence(timeout: 5), "Should return to Search view")
    }
}
