...
final class PulseSettingsUITests: BaseUITestCase {
    ...
    func testSettingsFlow() {
        ...
        XCTAssertTrue(
            navigationTitle.safeWaitForExistence(timeout: Self.defaultTimeout),
            "Settings navigation should exist"
        )

        let accountSection = app.staticTexts["Account"]
        XCTAssertTrue(accountSection.safeWaitForExistence(timeout: Self.defaultTimeout), "Account section should exist")

        let subscriptionSection = app.staticTexts["Subscription"]
        XCTAssertTrue(subscriptionSection.safeWaitForExistence(timeout: 5), "Subscription section should exist")

        let goPremiumText = app.staticTexts.matching(
            NSPredicate(format: "label CONTAINS[c] 'Premium' OR label CONTAINS[c] 'premium'")
        ).firstMatch
        XCTAssertTrue(goPremiumText.safeWaitForExistence(timeout: 5), "Premium section should be visible")
        ...
        XCTAssertTrue(notificationsToggle.safeWaitForExistence(timeout: 5), "Notifications toggle should exist")

        let breakingNewsToggle = app.switches["Breaking News Alerts"]
        XCTAssertTrue(breakingNewsToggle.safeWaitForExistence(timeout: 5), "Breaking News toggle should exist")
        ...
        XCTAssertTrue(homeNav.safeWaitForExistence(timeout: 5), "Should return to Home")
        ...
        XCTAssertTrue(systemThemeToggle.safeWaitForExistence(timeout: 5))
        ...
        if darkModeToggle.safeWaitForExistence(timeout: 2) {
            darkModeToggle.tap()
            darkModeToggle.tap()
        }
        ...
        if mutedSourcesButton.safeWaitForExistence(timeout: 3) {
            mutedSourcesButton.tap()
            let addSourceField = app.textFields["Add source..."]
            XCTAssertTrue(addSourceField.safeWaitForExistence(timeout: 2), "Add source field should appear")
        }
        ...
        if mutedKeywordsButton.safeWaitForExistence(timeout: 3) {
            mutedKeywordsButton.tap()
            let addKeywordField = app.textFields["Add keyword..."]
            XCTAssertTrue(addKeywordField.safeWaitForExistence(timeout: 2), "Add keyword field should appear")
        }

        let contentFiltersFooter = app.staticTexts["Muted sources and keywords will be hidden from all feeds."]
        XCTAssertTrue(contentFiltersFooter.safeWaitForExistence(timeout: 5), "Footer text should explain content filters")
        ...
        XCTAssertTrue(versionLabel.safeWaitForExistence(timeout: 5), "Version label should exist")

        let githubLink = app.buttons["View on GitHub"]
        XCTAssertTrue(githubLink.safeWaitForExistence(timeout: 5), "GitHub link should exist")
    }
}
