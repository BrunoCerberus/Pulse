...
final class PremiumGatingNonPremiumUITests: BaseUITestCase {
    ...
    func testFeedTabShowsPremiumGateForNonPremiumUser() {
        ...
        XCTAssertTrue(safeWaitForExistence(feedTab, timeout: Self.launchTimeout), "Feed tab should exist")
    ...
    func testFeedUnlockButtonShowsPaywall() {
        ...
        let unlockButton = app.buttons["unlockPremiumButton"]
        guard safeWaitForExistence(unlockButton, timeout: 10) else {
        ...
        let paywallAppeared = safeWaitForExistence(paywallTitle, timeout: 5) || safeWaitForExistence(subscriptionView, timeout: 5)
    ...
    func testSummarizeButtonShowsPaywallForNonPremiumUser() {
        ...
        guard safeWaitForExistence(summarizeButton, timeout: 5) else {
...
final class PremiumGatingPremiumUITests: BaseUITestCase {
    ...
    func testSummarizeButtonWorksForPremiumUser() {
        ...
        let paywallTitle = app.staticTexts["Unlock Premium"]
        XCTAssertFalse(safeWaitForExistence(paywallTitle, timeout: 2), "Premium user should not see paywall")
    ...
    private func navigateToArticleDetail() -> Bool {
        ...
        guard let card = firstCard, safeWaitForExistence(card, timeout: 10) else {
            return false
        }
