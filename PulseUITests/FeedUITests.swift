...
final class FeedUITests: BaseUITestCase {
    ...
    func testSourceArticleNavigation() {
        ...
        if safeWaitForExistence(sourceArticlesText, timeout: Self.defaultTimeout) {
            ...
            if buttonCount > 0 {
                ...
                if safeWaitForExistence(firstButton, timeout: Self.shortTimeout), firstButton.exists {
                    firstButton.tap()
    ...
    func testGenerateDigestInteraction() {
        ...
        if safeWaitForExistence(generateButton, timeout: Self.shortTimeout), generateButton.exists {
            generateButton.tap()
