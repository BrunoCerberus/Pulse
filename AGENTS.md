...
1. **UDF direction**: View → ViewModel → Interactor → Service. Views never touch services; ViewModels never touch APIs. *Sole exception:* the premium-gated views (`FeedView`, `ArticleDetailView`, `SettingsView`) hold a `ServiceLocator` and read `StoreKitService` directly to seed/re-verify `isPremium` — intentional defense-in-depth for entitlement checks (see rule 7), not a pattern to copy elsewhere.
...
7. **Premium gates** — views seed `isPremium` from the cached `StoreKitService.isPremium` (via `subscriptionStatusPublisher`) for first render, then re-verify against live StoreKit 2 entitlements via `refreshSubscriptionStatus()` in a `.task` at appearance (`FeedView`, `ArticleDetailView`'s summarize button, `SettingsView`). Defense-in-depth: `SummarizationDomainInteractor.startSummarization` and `FeedDomainInteractor.generateDigest` re-`guard` on the synchronous cached `isPremium` at the service boundary.
...
16. **Input validation at boundaries** — YouTube ID regex; HTTPS-only media/article-URL gate (`SafeMediaURL.validated`/`isSafe`) shared across the inline-video `WKWebView` (`VideoPlayerView`), podcast `AVURLAsset` (`AudioPlayerView`), and both open-in-browser sinks (`MediaDetail`/`ArticleDetailDomainInteractor`) so the scheme allowlist can't drift; untrusted RSS text routed through `PromptSanitizer.sanitize(_:maxLength:)` (`Configs/AI/`) in all three prompt builders before LLM interpolation (neutralizes Gemma turn markers, control chars → spaces, strips backticks, length-caps); deeplink ID allowlist + path-traversal rejection (`Deeplink.isValidArticleID`); free-text deeplink params (search query, category name) control-char-stripped + length-capped at the parse boundary via `Deeplink.sanitizedQueryParameter` in both `DeeplinkManager` and `NotificationDeeplinkParser`; disk cache filename sanitization; 256-char search limit (also enforced downstream in `SearchDomainInteractor`).
17. **Sign-out and account deletion** both call `SettingsViewModel.clearAllUserData()` — a single-flight (`isClearingUserData`) wipe covering SwiftData + L1+L2 cache + Keychain + UserDefaults + ThemeManager + widget/Share-Extension App Group data + TTS Live Activity + personalization profile & engagement queue + app-lock state, plus an APNs unregister and a best-effort private CloudKit zone delete (so reinstall-restore can't resurrect erased PII). The same wipe also fires from `AuthenticationManager` (wired in `PulseSceneDelegate.configureSessionCleanup`) on any authenticated→unauthenticated transition (server-driven sign-out / remote disable), de-duped by the single-flight guard. `AuthService.deleteAccount(presenting:)` handles `requiresRecentLogin` by reauthenticating with the original provider (Apple Guideline 5.1.1(v)). Firebase `User` calls use `withCheckedThrowingContinuation` so the non-Sendable `User` never crosses `await`.
...
34. **Reviewer-only anonymous sign-in** — `SignInView` exposes a hidden 5-tap-on-logo gesture that dispatches `.signInAnonymously` (Firebase Anonymous Auth) so App Review reaches auth-gated screens without shared OAuth credentials. The trigger is documented in App Store Connect → App Review Information. Hardened to keep reviewer sessions out of production analytics: a 60s throttle (UserDefaults key `pulse.anonymousSignInLastAttempt`, recorded only on confirmed Firebase success, survives sign-out) rejects rapid re-taps with `AuthError.anonymousSignInThrottled`, which `AuthDomainInteractor` special-cases to suppress the failed-`signIn` analytics + `recordError` calls; `PulseSceneDelegate.configureAnalyticsUserID()` sends `setUserID(nil)` for `provider == .anonymous` so no analytics identity is bound. The successful `signIn(provider: "anonymous")` event is still logged so reviewer sessions can be filtered out of DAU/retention dashboards. Implementation lives in `LiveAuthService+Anonymous.swift` (split from `LiveAuthService.swift` to stay under the SwiftLint file-length budget, same pattern as `FirebaseUser+AuthUser.swift`). Don't surface this gesture in onboarding, accessibility hints, or marketing copy — discoverability by real users is the only failure mode.
35. **Security review is threat-model-guided.** `THREAT_MODEL.md` (trust boundaries, untrusted inputs, vuln classes — backend is out of scope, it lives in `pulse-backend`) and `SEVERITY_RUBRIC.md` (five-dimension scoring; Medium default ceiling because most flows sit behind Firebase auth + a private CloudKit zone) scope all security work. `.github/workflows/claude-security-review.yml` runs an **advisory** two-stage pass on PRs — `pr-discovery` (emits `security-findings.json`, posts nothing) → `pr-verify` (separate job, no shared context, prompted to *disprove* each candidate, posts only confirmed) — plus a weekly full-repo sweep, distinct from the general-purpose `claude-code-review.yml`. The discovery↔verify split is deliberate: a self-verifying discovery agent filters out true positives a separate step would confirm. When you patch a validated finding, update `THREAT_MODEL.md` §3 to reflect the fixed state and add a regression test (the deeplink sanitizer in rule 16 is the worked example). Keep these advisory — do **not** wire them into the required-check ruleset (that coupling is reserved for the deterministic privacy gates in rule 33).
