...
20. **Sign-out / account deletion clears all user data** - `SettingsViewModel.clearAllUserData()` wipes SwiftData (bookmarks, preferences, reading history), L1+L2 caches, app lock keychain, recent searches, UserDefaults, ThemeManager, and widget shared data. Called by both the sign-out flow and the account deletion flow (Apple Guideline 5.1.1(v) requires in-app account deletion). For deletion, `AuthService.deleteAccount(presenting:)` handles Firebase's `requiresRecentLogin` by re-authenticating with the original provider (Google/Apple) before retrying. Never leave stale user data after either flow.
...
32. **Mac Catalyst is blocked at the SPM level** — `SUPPORTS_MACCATALYST: false` in `project.yml`. The vendored `swift-llama-cpp` xcframework has no Catalyst slice, so SPM fails to link it for Catalyst at the package target level — source-level `#if` guards aren't sufficient to unblock the flag. To enable Catalyst in the future: (1) rebuild `BrunoCerberus/swift-llama-cpp` with a `maccatalyst` xcframework slice (or fork the Package.swift to exclude Catalyst platforms), (2) put the Catalyst LLM stub in a standalone file (e.g. `LLMModelManagerCatalystStub.swift`) so the original `LLMModelManager.swift` stays untouched and doesn't count as changed against patch coverage, (3) flip `SUPPORTS_MACCATALYST: true`, (4) add `#if !targetEnvironment(macCatalyst)` guards around `ActivityKit` usage (`TTSLiveActivityController`, `TTSActivityAttributes`) and `UIApplicationShortcutItem` registration in `QuickActionHandler` — these APIs are iOS-only. Do NOT use the deprecated `DERIVE_MACCATALYST_PRODUCT_BUNDLE_IDENTIFIER`; conditionalize `PRODUCT_BUNDLE_IDENTIFIER` with `[sdk=macosx*]` instead.
