...
16. **Input validation at boundaries** — YouTube ID regex, HTTPS-only URL allowlist, deeplink ID allowlist + path-traversal rejection (`Deeplink.isValidArticleID`), free-text deeplink params (search query, category name) control-char-stripped + length-capped at the parse boundary via `Deeplink.sanitizedQueryParameter` in both `DeeplinkManager` and `NotificationDeeplinkParser`, disk cache filename sanitization, 256-char search limit (also enforced downstream in `SearchDomainInteractor`).
...
34. **Reviewer-only anonymous sign-in** — `SignInView` exposes a hidden 5-tap-on-logo gesture that dispatches `.signInAnonymously` (Firebase Anonymous Auth) so App Review reaches auth-gated screens without shared OAuth credentials. The trigger is documented in App Store Connect → App Review Information; `AuthDomainInteractor` logs the analytics event with `provider: "anonymous"` so reviewer sessions can be filtered out of DAU/retention dashboards. Implementation lives in `LiveAuthService+Anonymous.swift` (split from `LiveAuthService.swift` to stay under the SwiftLint file-length budget, same pattern as `FirebaseUser+AuthUser.swift`). Don't surface this gesture in onboarding, accessibility hints, or marketing copy — discoverability by real users is the only failure mode.
