...
def get_changed_files(base_sha: str, head_sha: str) -> list[str] | None:
    """Return the list of file paths changed by the PR (merge-base diff).

    Returns ``None`` when git fails — the caller should treat that as
    "run UI tests" to stay fail-safe.
...
def main():
    ...
    if changed_files is None:
        print("Git diff failed; running UI tests to be safe.")
        set_output("true")
        return
